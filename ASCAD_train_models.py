...
from tensorflow.keras.layers import Flatten, Dense, Input, Conv1D, MaxPooling1D, GlobalAveragePooling1D, GlobalMaxPooling1D, AveragePooling1D, BatchNormalization, Activation, Add, add, Dropout # type: ignore
...
def mlp_best(node=200, layer_nb=6, input_dim=1400):
    ...
    model = Model(inputs=inputs, outputs=outputs, name='mlp_best')
...
def cnn_best(classes=256, input_dim=700):
    ...
    x = Dropout(0.2)(x)  # Add dropout with a rate of 20%
    ...
    x = Dropout(0.4)(x)  # Add dropout with a rate of 40%
...
def resnet_layer(inputs,
                 ...
                 strides=1,
                 activation='relu',
                 batch_normalization=False, # batch normalization enabled
                 conv_first=True):
    ...
    if conv_first:
        x = conv(x)
        if batch_normalization:             # skips batch normalization
...
def train_model(X_profiling, Y_profiling, model, save_file_name, epochs=150, batch_size=100, multilabel=0, validation_split=0, early_stopping=0):
    ...
    # Update save file name to use .keras extension
    save_file_name = save_file_name.replace('.h5', '.keras')
    # Save model callback
    ...
    if input_layer_shape[1] != len(X_profiling[0]):
        ...
        sys.exit(-1)
    # Adapt the data shape according to our model input
    ...
    history_file = save_file_name.replace('.keras', '_history.pkl')
...
if __name__ == "__main__":
    if len(sys.argv)!=2:
        ...
        #CNN training w/ batch normalization
        #network_type = "cnn"
        #training_model = "ATMEGA_AES_v1/ATM_AES_v1_fixed_key/ASCAD_data/ASCAD_trained_models/my_cnn_best_bn_desync0_epochs5_batchsize200.h5"

        #CNN training w/ no batch normalization
        #network_type = "cnn"
        #training_model = "ATMEGA_AES_v1/ATM_AES_v1_fixed_key/ASCAD_data/ASCAD_trained_models/my_cnn_best_no_bn_desync0_epochs5_batchsize200.h5"

        #CNN training Dropout
        network_type = "cnn"
        training_model = "ATMEGA_AES_v1/ATM_AES_v1_fixed_key/ASCAD_data/ASCAD_trained_models/my_cnn_best_dropout_desync0_epochs5_batchsize200.h5"
        ...
        multilabel = 0
        train_len = 0
        epochs = 5         # changed from 75 to 20 to 5
        batch_size = 200
        bugfix = 0
        early_stopping = 10  # Default value for early stopping
    else:
